...
function mylistChanged() {
	numberOfNearestStations = document.getElementById("mylist").value;
	window.localStorage.setItem("numberOfNearestStations", numberOfNearestStations);	
	initialize();
}

function setList() {	
	var mylistValue = ["3","4","5","6","7","8","9","10"];
	var mylistIndex = ["0","1","2","3","4","5","6","7"];
	
	for(var i = 0 ; i <= 7 ; i ++){
		if(numberOfNearestStations == mylistValue[i]){
			document.getElementById("mylist").selectedIndex = mylistIndex[i];
			break;
		}
	}
}
...
function checkData() {
	var number = window.localStorage.getItem("numberOfNearestStations");
	
	if(number == null){
		numberOfNearestStations = 5;
		window.localStorage.setItem("numberOfNearestStations", numberOfNearestStations);
	}		
	else{
		numberOfNearestStations = number;
	}
	setList();
}
  ...
  function initArray(){		   
   ...
   function showResultsOnMap(map){
	...
	var openInfoWindow = function(marker1, index, address1) {
           return function() {	
		
		var markerLatLng = marker1.getPosition();
		var number = "Top "+(index+1);
		content = address1 +"<BR>" + number;
